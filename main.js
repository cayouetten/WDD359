...
search.addEventListener('click', function(e) {
	...
	{
		createCharacterRequest(apiRequest, option);
	}
});

function createCharacterRequest(url, option) {
	...
	request.onload = function() {
		if(request.status >= 200 && request.status < 400) {
			...
			//Update page with new location content
			var res = document.querySelector('.resultDiv');
			//IF statement to ensure element available before parse
			if(res != null) {
